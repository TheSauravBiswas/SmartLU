...
$totalcr=0;
$cr[0]=0;
$selectreg=$resultsreg=NULL;
$percr = $semseaErr=$semyearErr=$dbErr="";
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $completecr = $_SESSION['logger']['completecr'];
    $currentcr = $_SESSION['logger']['currentcr'];
	$semester = $_POST['semsea']."-".$_POST['semyear'];

    for($j=1; $j<5; $j++){
        if(isset($_POST['ctitle'.$j])){
            ${'ctitle'.$j} = $_POST['ctitle'.$j];
            
            if($_POST['ctitle'.$j]=="Theory of Computation"){
                ${'ccode'.$j} = "CSE-2201";
                $cr[$j] = 3;
            }elseif($_POST['ctitle'.$j]=="Data Communications"){
                ${'ccode'.$j} = "CSE-2231";
                $cr[$j] = 3;
            }elseif($_POST['ctitle'.$j]=="Digital Logic Design"){
                ${'ccode'.$j} = "CSE-2221";
                $cr[$j] = 3;
            }elseif($_POST['ctitle'.$j]=="Computer Networks"){
                ${'ccode'.$j} = "CSE-3115";
                $cr[$j] = 3;
            }elseif($_POST['ctitle'.$j]=="Course Codes: Course Titles (Credits)"){
                ${'ctitle'.$j} = "none";
                ${'ccode'.$j} = "none";
                $cr[$j] = 0;
            }
    ...
    for($k=1; $k<5; $k++){
        $totalcr = $totalcr +  $cr[$k];
        ${'cr'.$k} = $cr[$k];
    }
    $percr = $_SESSION['logger']['percr'] * 120 / 100;
    $totalfee = $percr * $totalcr;
    $selectreg = " SELECT * FROM supplement WHERE sid = '$sid' ";
    $resultsreg = mysqli_query($conn, $selectreg);

    if(mysqli_num_rows($resultsreg) > 0){
        $dbErr="* already applied";
    }elseif($_POST["semsea"]=="Choose...") {
        ...
        $sql = "INSERT INTO supplement(name, sid, dept, program, completecr, currentcr, semester, ccode1, ctitle1, cr1,  ccode2, ctitle2, cr2, ccode3, ctitle3, cr3, ccode4, ctitle4, cr4, totalcr, hstatus, dfa, cstatus, totalfee, fees, assigned) 
                VALUES('$name', '$sid', '$dept', '$program', '$completecr', '$currentcr', '$semester', '$ccode1', '$ctitle1', '$cr1', '$ccode2', '$ctitle2', '$cr2', '$ccode3', '$ctitle3', '$cr3', '$ccode4', '$ctitle4', '$cr4', '$totalcr', 'Pending', 'Pending', 'Pending', '$totalfee', '0', 'Pending')";
        
        $result = mysqli_query($conn, $sql);

        if($result){
            header("location:suppleStatus.php?success=We have received your application.");
            ob_end_flush();
            exit;
        }
        else{
...
<nav class="navbar navbar-expand-lg navbar-dark fixed-top">
    ...
    <div class="container">
    <h2 style="padding-top: 75px;">Supplementary Application Form</h2>
        <?php if ($dbErr) { ?>
            <div class="alert alert-danger" role="alert">
                <?php echo $dbErr;?>
            </div>
        <?php } ?>
        <form id="fields-list" class="admissionform my-3" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method ="post" enctype='multipart/form-data'>
            ...
            <div class="form-group col-md-12 ">
                <label for="ctitle">Course <?=$i?></label>
                <select id="ctitle<?=$i?>" class="form-select" aria-label="Default select example" name="ctitle<?=$i?>">
                    <option selected>Course Codes: Course Titles (Credits)</option>
                    <option value="Theory of Computation" >CSE-2201: Theory of Computation (3)</option>
                    <option value="Data Communications" >CSE-2231: Data Communications (3)</option>
                    <option value="Digital Logic Design" >CSE-2221: Digital Logic Design (3)</option>
                    <option value="Computer Networks" >CSE-3115: Computer Networks (3)</option>
                </select>
            </div>
            </div>
            ...
            <button type="submit" class="btn btn-primary" name="supplement">Submit</button>
        </form>
