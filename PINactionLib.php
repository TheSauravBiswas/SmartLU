...
if(mysqli_num_rows($result) > 0){
    ...
    if($row['balance']>=$fees){
        ...
        $sql2 = "INSERT INTO library(name, sid, dept, program, batch, fees, hstatus, dregistrar, librarian) VALUES('$name', '$id', '$dept', '$program', '$batch', '$fees','Pending', 'Pending', 'Pending')";
        $resultsql2 = mysqli_query($conn, $sql2);

        header("location:index2.php?success=Library fees payment was successful!");
        ob_end_flush();
        exit;
    }
    else{
        header("location:index2.php?error=Library fees payment was unsuccessful! Insufficient balance.");
        ob_end_flush();
        exit;
    }
}
