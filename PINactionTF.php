...
if(mysqli_num_rows($result) > 0){
    ...
    if($row['balance']>=$fees){
        ...
        $tfsql = " SELECT * FROM tuitionfees WHERE id='$id' ";
        $tfrow = mysqli_fetch_array(mysqli_query($conn, $tfsql));
        $newfees = $tfrow['fees'] + $fees;
        $sql2 = "UPDATE tuitionfees SET fees='$newfees' WHERE id='$id' ";
        $resultsql2 = mysqli_query($conn, $sql2);

        header("location:tuitionFees.php?success=Payment successful!");
        ob_end_flush();
        exit;
    }
    else{
        header("location:tuitionFees.php?error=Payment unsuccessful! Insufficient balance.");
        ob_end_flush();
        exit;
    }
}
