...
if($myStatus=="Head"){
    $sql = "SELECT * FROM supplement WHERE dept = '$myDept' ORDER BY id DESC";
    $result = mysqli_query($conn, $sql);
}elseif($myIndex=="dfa"){
    $sql = "SELECT * FROM supplement WHERE hstatus = 'Approved' ORDER BY id DESC";
    $result = mysqli_query($conn, $sql);
}elseif($myStatus=="Controller of Examinations"){
    $sql = "SELECT * FROM supplement WHERE dfa = 'Approved' ORDER BY id DESC";
    $result = mysqli_query($conn, $sql);
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>LU | Supplementary Applications</title>
    ...
    <?php if (mysqli_num_rows($result)) { ?>
        <table class="table table-light my-3">
            <thead>
                ...
                <th scope="col">#</th>
                <th scope="col">ID</th>
                <?php if($myStatus=="Head"||$myIndex=="dfa"||$myStatus=="Controller of Examinations"){ ?>
                <th scope="col">Name</th><?php } ?>
                ...
                <?php if($myStatus=="Head"||$myIndex=="dfa"||$myStatus=="Controller of Examinations"){ ?>
                <th scope="col">You</th><?php }else{ ?>
                <th scope="col">Assign Teacher</th><?php } ?>
                </tr>
            </thead>
            <tbody>
                <?php 
                ...
                <tr>
                    ...
                    <td rowspan="4"><?php echo $rows['sid']; ?></td>
                    <?php if($myStatus=="Head"||$myIndex=="dfa"||$myStatus=="Controller of Examinations"){ ?>
                    <td rowspan="4"><?php echo $rows['name']; ?></td><?php } ?>
                    <td rowspan="4"><?php echo $rows['semester']; ?></td>
                    <td rowspan="4"><?php echo $rows['program']; ?></td>
                    <td><?php if($rows['ccode1']=="none"){echo "";}else{echo $rows['ccode1'];} ?></td>
                    <td><?php if($rows['ctitle1']=="none"){echo "";}else{echo $rows['ctitle1'];} ?></td>
                    <td><?php if($rows['cr1']==0){echo "";}else{echo $rows['cr1'];} ?></td>
                    
                    <td rowspan="4">
                    <?php if($myStatus=="Head"){ ?>
                    <a href="approveSupple.php?col=hstatus&id=<?=$rows['id']?>" class="btn btn-success m-1 d-grid" 
                    <?php if($rows['hstatus']=='Approved'){echo "hidden";} ?>>Approve</a>
                    <a href="declineSupple.php?col=hstatus&id=<?=$rows['id']?>" class="btn btn-danger m-1 d-grid" 
                    ...
                    <a href="approveSupple.php?col=dfa&id=<?=$rows['id']?>" class="btn btn-success m-1 d-grid" 
                    <?php if($rows['dfa']=='Approved'){echo "hidden";} ?>>Approve</a>
                    <a href="declineSupple.php?col=dfas&id=<?=$rows['id']?>" class="btn btn-danger m-1 d-grid" 
                    ...
                    <a href="approveSupple.php?col=cstatus&id=<?=$rows['id']?>" class="btn btn-success m-1 d-grid" 
                    <?php if($rows['cstatus']=='Approved'){echo "hidden";} ?>>Approve</a>
                    <a href="declineSupple.php?col=cstatus&id=<?=$rows['id']?>" class="btn btn-danger m-1 d-grid" 
                ...
                <?php for($i=2;$i<5;$i++){
                    if($rows['ctitle'.$i]=="none"){
                        continue;
                    } ?>
                <tr>
                    <td><?php echo $rows['ccode'.$i]; ?></td>
                    <td><?php echo $rows['ctitle'.$i]; ?></td>
                    <td><?php echo $rows['cr'.$i]; ?></td>
                </tr>
                <?php }} ?>
            </tbody>
        </table>
