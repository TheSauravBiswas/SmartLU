...
if(mysqli_num_rows($result) > 0){
    ...
    if($row['balance']>=$fees){
        ...
        $sql2 = "UPDATE grad SET fees='$newfees' WHERE id='$id' ";
        $resultsql2 = mysqli_query($conn, $sql2);

        header("location:gradStatus.php?success=Payment successful!");
        ob_end_flush();
        exit;
    }
    else{
        header("location:gradStatus.php?error=Payment unsuccessful! Insufficient balance.");
        ob_end_flush();
        exit;
    }
}
