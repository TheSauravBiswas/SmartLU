...
if(mysqli_num_rows($result) > 0){
    ...
    if($row['balance']>=$fees){
        ...
        $sql2 = "UPDATE makeup SET fees='$fees' WHERE id='$id' ";
        $resultsql2 = mysqli_query($conn, $sql2);

        header("location:makeupStatus.php?success=Payment successful! You can download admit card now.");
        ob_end_flush();
        exit;
    }
    else{
        header("location:makeupStatus.php?error=Payment unsuccessful! Insufficient balance.");
        ob_end_flush();
        exit;
    }
}
